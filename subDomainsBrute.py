...
class SubNameBrute(object):
    def __init__(self, *params):
        self.domain, self.options, self.process_num, self.dns_servers, self.next_subs, \
            self.scan_count, self.found_count, self.queue_size_array, tmp_dir ,self.wildcard_ip= params
        ...
        self.load_sub_names()
        self.lock = RLock()
        self.wildcard_ip = self.wildcard_ip
        ...
        while True:
            ...
            try:
                ...
                if answers:
                    ...
                    # print ips.strip()
                    # print "go"
                    # print self.wildcard_ip
                    # print '\n'
                    #wildcard_ip为泛域名解析IP
                    if ips.strip() == self.wildcard_ip.strip():
                        pass
                    else:
                        #self.outfile.write(cur_domain.ljust(30).strip() + '\n')
...
if __name__ == '__main__':
    ...
    '''
    python subDomainsBrute.py 1 1 1
    options,args默认输出为一下：
    {'full_scan': False, 'process': 6, 'i': False, 'threads': 200, 'file': 'subnames.txt', 'output': None}
    ['1', '1', '1']
    '''
...
'''
    ...
    try:
        ...
        domain = args[0]
        wildcard_ip = wildcard_deal(args[0])
        ...
        for process_num in range(options.process):
            p = multiprocessing.Process(target=run_process,
                                        args=(domain, options, process_num, dns_servers, next_subs,
                                              scan_count, found_count, queue_size_array, tmp_dir, wildcard_ip)
                                        )
    ...
    with open(out_file_name, 'w') as f:
        #tmp_dir = 'tmp/%s_%s' % (args[0], int(time.time())),先把结果写在这个文件里面
